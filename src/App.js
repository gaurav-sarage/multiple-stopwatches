      ...
      <div className="container">
        <div className="left">
          ...
          <p className="main-time-label">
            {stopwatchData[selectedRow]?.time > 0 ? new Date(stopwatchData[selectedRow]?.time).toISOString().slice(11, -1) : "00:00:00.000"}
          </p>

          <div className="main-button-controls">
            <button className={"control-buttons"} onClick={() => handleStart(stopwatchData[selectedRow].id)}>
              Start
            </button>

            <button className={"control-buttons"} onClick={() => handleLap(stopwatchData[selectedRow].id)}>
              Lap
            </button>

            <button className={"control-buttons"} onClick={() => handleStop(stopwatchData[selectedRow].id)}>
              Stop
            </button>

            <button className={"control-buttons"} onClick={() => handleReset(stopwatchData[selectedRow].id)}>
              Reset
            </button>
          </div>
          <p className="lap-label">
            Laps
          </p>
